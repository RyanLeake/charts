...
public struct PieChartRow: View {
    ...
    var slices: [PieSlice] {
        ...
        for (index, slice) in chartData.points.enumerated() {
            let normalized: Double = Double(slice) / (maxValue == 0 ? 1 : maxValue)
            let startDeg = lastEndDeg
            let endDeg = lastEndDeg + (normalized * 360)
            lastEndDeg = endDeg
            tempSlices.append(PieSlice(startDeg: startDeg, endDeg: endDeg, value: slice, color: chartData.colors[index]))
        }
            ...
            ZStack {
                ForEach(0..<self.slices.count) { index in
                    PieChartCell(
                        ...
                        accentColor: self.slices[index].color
                    )
